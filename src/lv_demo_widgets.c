...
static void perf_monitor_auto_off_cb(lv_timer_t * t);
#endif
...
static void sd_list_iter_cb(const char * name, bool is_dir, uint32_t size, void * ctx);
...
static void settings_create(lv_obj_t * parent)
{
    ...
    lv_obj_set_flex_align(perf_cont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

    lv_obj_t * perf_chk = lv_checkbox_create(perf_cont);
    lv_checkbox_set_text(perf_chk, "Performance Monitor (FPS/CPU)");
    lv_obj_add_event_cb(perf_chk, perf_monitor_switch_cb, LV_EVENT_VALUE_CHANGED, NULL);
    /*Default ON*/
    lv_obj_add_state(perf_chk, LV_STATE_CHECKED);
    /*Ensure label visible if found*/
    lv_obj_t * lbl = perf_monitor_find_label();
    if(lbl) {
        lv_obj_clear_flag(lbl, LV_OBJ_FLAG_HIDDEN);
    }

    /*Auto-turn-off after 2 seconds from start*/
    lv_timer_create(perf_monitor_auto_off_cb, 100, perf_chk);
#endif
    ...
    lv_obj_add_event_cb(bl_slider, backlight_slider_cb, LV_EVENT_VALUE_CHANGED, NULL);
    ...
    lv_obj_set_flex_align(reset_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

    lv_obj_t * reset_btn = lv_btn_create(reset_row);
    lv_obj_set_width(reset_btn, lv_pct(100));  /*Full width since no Rotate button*/
...
static void sd_update_mount_button(void) {
    if(!sd_mount_label) return;
    if(sd_card_is_mounted()) lv_label_set_text(sd_mount_label, "Unmount SD");
    else lv_label_set_text(sd_mount_label, "Mount SD");
}

static void sd_update_status(const char * msg) {
    if(sd_status_label) lv_label_set_text(sd_status_label, msg);
}
...
static void sd_card_tab_create(lv_obj_t * parent)
{
    ...
    lv_obj_add_event_cb(sd_mount_btn, sd_mount_btn_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(refresh_btn, sd_refresh_btn_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(sd_format_btn, sd_format_btn_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(test_log_btn, sd_test_log_btn_event_cb, LV_EVENT_CLICKED, NULL);
...
static void perf_monitor_auto_off_cb(lv_timer_t * t)
{
    ...
    if(chk) {
        ...
        /*Trigger handler to hide label*/
        lv_event_send(chk, LV_EVENT_VALUE_CHANGED, NULL);
    }
    lv_timer_del(t);
}
#endif
...
static void backlight_slider_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        *s = LV_MAX(*s, 60);
        ...
        if(dsc->part == LV_PART_KNOB && lv_obj_has_state(slider, LV_STATE_PRESSED)) {
            char buf[8];
...
static void sd_list_item_cb(lv_event_t * e) {
    ...
    if(index >= sd_entry_count) return;
    ...
    if(entry->is_directory) {
        ...
        char buf[64];
        lv_snprintf(buf, sizeof(buf), "File: %s (%lu bytes)", entry->name, (unsigned long)entry->size);
...
static void sd_refresh_list(void) {
    ...
    for(int i = 0; i < sd_entry_count; i++) {
        ...
        if(sd_entries[i].is_directory) {
            ...
            lv_snprintf(label, sizeof(label), "%s (%lu B)", sd_entries[i].name, (unsigned long)sd_entries[i].size);
...
static void sd_test_log_btn_event_cb(lv_event_t * e) {
    ...
    log_info("TEST", "Test log entry #%lu from SD Card tab", (unsigned long)test_counter);
